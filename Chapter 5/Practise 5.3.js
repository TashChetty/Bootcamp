...
let myWork =[];             //blank array
for (let i = 1; i < 10; i++) {                  // for loop, variable, condition and statement
    let lessonStatus = i % 2 ? true : false;    //trenary operator to specify lesson status using boolean
    let temp = {                                //temp variable with lesson name and numeric value
        name: `Lesson ${i}`, status: lessonStatus
    };
    myWork.push(temp);                          //adding the lesson and lesson status to the array
}
console.log(myWork);                            // logging to console.