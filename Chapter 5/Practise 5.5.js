...
let grid =[];           //grid array 
let cells = 64;         // cells variable set to 64
let counter = 0;        // counter set to 0
var row ;               //global variable for row
for(let i = 0; i < cells + 1; i++){     //  loop for number of cells +1 to show zero value
    if (counter % 8==0){                //if statement --modulo to check if the main counter is divisible by 8 number of columns and value of counter is equal to total number of columns
        if (row != undefined){          // if statement checking if row is undefined
            grid.push(row);             // if row is defined add to main grid array
        }
        row = [];
    }
    counter++
    ...
    let temp = counter;                 // pushing to row array
    row.push(temp);
 

console.table(grid);                    //output grid
